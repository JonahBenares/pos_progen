<div class="main-panel">
    <div class="content-wrapper">    
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-gradient-primary card-img-holder text-white">
                        <h4 class="m-0">Add Item</h4>
                    </div>
                    <div class="card-body">                        
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label >Item Description</label>
                                    <select class="form-control">
                                        <option>-Select Item-</option>
                                    </select>
                                </div>
                            </div>                             
                        </div>  
                        <div class="row">
                            <div class="col-md-6 col-sm-6">
                                <div class="form-group">
                                    <label >Qty</label>
                                    <input type="number" class="form-control amount-txt" name="" placeholder="00.00">
                                </div>
                                <div class="form-group">
                                    <label >Item Code</label>
                                    <input type="text" class="form-control" name="" placeholder="Item code">
                                </div>
                                                               
                            </div>
                            <div class="col-md-6 col-sm-6">
                                <div class="form-group">
                                    <label >UOM</label>
                                    <input type="text" class="form-control" name="" placeholder="Unit of Measurement">
                                </div> 
                                <div class="form-group">
                                    <label >Price per Unit</label>
                                    <input type="text" class="form-control amount-txt" name="" placeholder="00.00">
                                </div>
                            </div>                            
                        </div>
                        <div class="row">     
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <div class="form-group">
                                    <label >Total Amount</label>
                                    <input type="text" class="form-control amount-txt" name="" placeholder="00.00">
                                </div>
                            </div>                       
                        </div>
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="pull-right">
                                    <input type="submit" class="btn btn-gradient-primary btn-md" value="Add Item">
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>       
    </div>
</div>
        
                        




